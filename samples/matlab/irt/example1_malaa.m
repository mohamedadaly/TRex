...
%% Read the SL phantom from ASTRA
% dd = load('/media/sf_I_DRIVE/Temp/astra-1.7.1beta-src/sl.mat');
dd = load('I:/Temp/astra-1.7.1beta-src/sl.mat');

A = dd.proj_mat;
params.clim = [0, 1];
params.ig.mask = true(size(dd.P));
params.xini = dd.fbp;

wi = ones(size(dd.sinogram)); % PWLS weights; gives 0 weight to any ray where yi=0!
mxW = max(wi(:));
mnW = min(wi(:));
params.Wy = dd.sinogram;
params.W = wi;

params.Nmask = sum(params.ig.mask(:)>0);
params.img = dd.P;

nx = dd.vol_geom.GridColCount;
ny = dd.vol_geom.GridRowCount;
nxs = dd.proj_geom.DetectorCount;
nys = length(dd.proj_geom.ProjectionAngles);
N = [nx ny];
farg.A = A;
farg.ny = ny;
farg.nx = nx;
farg.nys = nys;
farg.nxs = nxs;
params.A = Fatrix(size(A), farg, 'caller', 'malaa', ...
...
% if ~isvar('kappa'), printm('kappas')
	kappa = sqrt( div0(params.A' * wi, params.A' * ones(size(wi))) );
	skap = sort(kappa(:));
	skap = skap(skap > 0);
...
Acm = dd.proj_mat(rids, cids);
% convert to blocks with 40 subsets
...
% SQS-OS
% [xos timos] = pwls_sqs_os(params.xini, sos.Ab, dd.sinogram, ...
%   sos.R, 'wi', wi, 'niter', 10, 'pixmax', [0 inf], ...
%   'isave', 'all', 'chat', 1, 'relax0',[1 1e-5]);  %[1 1e-5]

% % SQS with momentum
% [xos timos] = pwls_sqs_os(params.xini, sos.Ab, dd.sinogram, ...
%   sos.R, 'wi', wi, 'niter', 10, 'pixmax', [0 inf], ...
%   'isave', 'all', 'chat', 1, 'mom',2,  'relax0',[1 1e-2]);  

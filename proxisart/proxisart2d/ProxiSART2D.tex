...
\title{A \textbf{T}omography \textbf{Re}construction Pro\textbf{x}imal Framework
for Robust Sparse View X-Ray Applications}


\author{Mohamed Aly, \IEEEmembership{Member, IEEE},%
\thanks{M. Aly is with the Visual Computing Center, KAUST, KSA% and % the Computer Eng. Dept, Cairo University, Egypt%%
}, Guangming Zang, \IEEEmembership{Student Member, IEEE}, %
\thanks{G. Zang is with the Visual Computing Center, KAUST, KSA%
} Wolfgang Heidrich, \IEEEmembership{Member, IEEE}, %
\thanks{W. Heidrich is with the Visual Computing Center, KAUST, KSA%
} and Peter Wonka, \IEEEmembership{Member, IEEE}, %
...
and show superior quality on both synthetic and real datasets. \end{abstract}
...
around for years. In fact, one of the first implemented tomography
reconstruction algorithm was an iterative one, the Algebraic Reconstruction
Technique (ART) \cite{kak2001principles,gordon1970algebraic,gordon1971reconstruction}.
...
In fact, most commercial X-ray CT scanners employ some variant of
...
ART \cite{andersen1984simultaneous,andersen1989algebraic}, SIRT (Simultaneous
...
ART, BICAV, and OS-SQS, and show how to use these proximal operators
to minimize different data fitting terms, including least squares
(LS) that assumes a Gaussian noise model, and weighted least squares
(WLS) that assumes an approximation to a Poisson noise model \cite{clinthorne1993preconditioning}.
We also show how to plug in different powerful regularizers, namely
the Isotropic Total Variation (ITV) \cite{rudin1992nonlinear}, Anisotropic
...
the ADMM method from \cite{ramani2012splitting} and the OS-SQS method
(with and without momentum method) \cite{kim2015combining}, and show
that our framework gives superior reconstruction quality.
...
Backprojection (FBP) algorithm introduced \cite{herman2009fundamentals,kak2001principles}.
...
the reconstruction problem. They generally model the problem as a
linear system and solve it using established numerical methods \cite{herman2009fundamentals}.
ART, and its many variants, are among the best known iterative reconstruction
algorithms \cite{gordon1970algebraic,lent1977convergent,shepp1982maximum,censor1983finite,andersen1984simultaneous,andersen1989algebraic}.
...
information into the reconstruction process. For example, instead
of assuming a Gaussian noise model and minimizing a least squares
data term, one can easily use iterative methods with other noise models.
For example, a Poisson noise model \cite{clinthorne1993preconditioning,depierro1994modified,elbakri2002statistical,wang2006penalized,thibault2007three}.
boils down to solving WLS problem instead. Priors are also easy to
use with iterative methods. For example, the Total Variation \cite{rudin1992nonlinear}
...
They have also been used in tomography reconstruction. For example,
\cite{mory2012ecg} used the Alternating Direction Method of Multipliers
(ADMM) \cite{boyd2011distributed} with total variation prior, where
the data term was optimized using CG \cite{bjorck1996numerical}.
...
priors. \cite{ramani2012splitting} used ADMM with Preconditioned
CG (PCG) \cite{fessler1999conjugate} for optimizing the weighted
least squares data term. \cite{nien2015fast} used Linearized ADMM
\cite{parikh2013proximal} (also known as Inexact Split Uzawa \cite{esser2010general})
with Ordered Subset-based methods \cite{erdogan1999ordered} for optimizing
the data term and FISTA \cite{beck2009fast} for optimizing the prior
term. However, none of these methods used SART as their data term
solver, which provides superior reconstruction as we will next.

There are currently a number of open source software packages for
tomography reconstruction. SNARK09 \cite{klukowska2013snark09} is
one of the oldest. The Reconstruction ToolKit (RTK) \cite{rit2014reconstruction}
...
It includes implementations of several algorithms, including FDK,
SART, and an ADMM TV-regularized solver with CG \cite{mory2012ecg}.
The ASTRA toolbox \cite{van2015astra} is a Matlab-based GPU-accelerated
...
one subset $S=\{1,\ldots,m\}$. The update step $\Delta x^{(t)}$
is typically a function of (a subset of) the forward projection error
$p_{S}-A_{S}x^{(t)}$ that is then back projected with some normalization
procedure. It can take the form 
\[
\Delta x^{(t)}=\Phi\left(A_{S}^{T},p_{S}-A_{S}x^{(t)}\right)
\]
where the function $\Phi(\cdot)$ computes the required update, see
below. This can be seen as an approximation to the actual gradient
...
Below we will quickly review the different methods, and Table \ref{tab:Iterative-Methods}
...
is the relaxation parameter. This update is performed once each row
of $A$, and one iteration includes a full pass over all the $m$
...
whore $c_{j}=\sum_{i}a_{ij}$ is the sum of column $j$ of $A$ and
...
assume that SART solves approximately the minimum-norm solution 
\[
...
column sums are now independent of the iteration. The matrix formulation
becomes 
\[
x^{(t+1)}=x^{(t)}+\alpha C^{-1}A_{S}^{T}R^{-1}\left(p_{S}-A_{S}x^{(t)}\right)
\]
 where the diagonal matrices are both independent of the projection
view $S$ as in SIRT. This has been shown \cite{censor2002block}
...
$C_{S}=\diag(c_{j}^{S})$. It is shown \cite{censor2002block} that
BICAV converges to the minimum-norm solution 
\[
...
like SIRT. SQS has been shown \cite{erdogan1999ordered} to converge
to a least square solution 
\[
x^{\star}=\argmin_{x}\Vert Ax-p\Vert_{2}^{2},
\]
 and special case of \emph{relaxed} OS-SQS converges, where the relaxation
parameter becomes iteration-dependent and decreases over time \cite{ahn2003globally}.
However, OS-SQS with fixed $\alpha$ is not known to converge. Therefore,
like SART, we assume that it solves the LS problem above approximately. 
...
\end{equation}
Now introduce new variables as follows: let $y=\sqrt{2\lambda}(p-Ax)$
and $z=x-u$. The problem becomes 
\begin{eqnarray}
...
\ref{tab:Prox-Operators} and Alg. \ref{alg:ART-like-Prox-Operator}.
...
in the form of a the LS problem that can be solved (approximately)
by OS-SQS: 
\[
x^{\star}=\argmin_{x}\Vert Ax-p\Vert_{2}^{2}.
\]
...
The steps are summarized in Table \ref{tab:Prox-Operators} and Alg.
\ref{alg:OS-SQS-Prox-Operator}.
...
The overall problem we are interested in solving is a regularized
data fitting problem. In particular, the problem we are interested
in solving is 
\begin{equation}
...
\st & Kx=z
\end{align*}
 and iterates the steps in Alg. \ref{alg:LADMM-algorithm}. This framework
is very flexible, and we will show how to solve for different data
terms and different regularizers. It is convergent for any $\rho>0$
and $\mu>\nicefrac{1}{\rho\Vert K\Vert^{2}}$ \cite{parikh2013proximal,nien2015fast}.
...
\end{equation}
where $\varepsilon\sim\mathbb{N}(0,1)$. The projection data log-likelihood
is
\begin{equation}
\mathcal{L}_{\text{G}}(p)\propto-\sum_{i}\left(p_{i}-a_{i}^{T}x\right)^{2}\label{eq:Gaussian-Log-Likelihood}
\end{equation}
and maximizing $\mathcal{L}_{\text{G}}(p)$ is equivalent to minimizing
the LS $\ell_{2}$ norm data term
\begin{equation}
...
Assuming an \emph{approximated} Poisson noise model leads to a WLS
data term, where the weights are proportional to the detector measurements
\cite{clinthorne1993preconditioning,depierro1994modified,elbakri2002statistical,thibault2007three}.
...
is proportional to the measurement of the incident intensity on detector
$i$ i.e.
\[
w_{i}\propto I_{t}^{i}.
\]
 Typically, the weights $w_{i}$ are normalized to have a maximum
of 1, and we could apply any non-decreasing mapping on $w_{i}$, e.g.
the square root, before feeding into the optimization problem, see
Sec. \ref{sub:Poisson-Mapping-Functions-Comparison}. 
...
It is a simplification of ITV \cite{sidky2008image,beck2009bfast,chambolle2010introduction,nien2015fast},
and is defined as 
\begin{equation}
h_{\text{ATV}}(x)=\sigma\Vert\nabla x\Vert_{1}\label{eq:h-ATV}
\end{equation}
 which is the $\ell_{1}$ norm of the gradient $\nabla x$ of the
volume. This can be written in the form 
\[
...
\footnote{available from \href{http://www.exxim-cc.com/}{www.exxim-cc.com}%
...
code. The iterative algorithms not present in ASTRA, namely BICAV,
BSSART, and OS-SQS, were implemented in C++. The proximal operators
were also implemented in C++. The Linearized ADMM was implemented
in Matlab. We also modified existing algorithms in ASTRA to suit our
needs e.g. compute SNR, report run times, etc. All experiments were
run on one core of an Intel Xeon E5-280 2.7 GHz with 64 GB RAM. 
...
\includegraphics[width=1\textwidth]{plots/per_iter-ph_ncat-512_nt_poisson-nl_100000\lyxdot 000-np_90-p_fan-snr}%
\end{minipage}
...
subsets with momentum. We don't compare to the method of Nien et al.
...
\footnote{available from \href{http://web.eecs.umich.edu/~fessler/code/}{web.eecs.umich.edu/$\sim$fessler/code/}%
...
\]
 where $\psi(\cdot)$ is the Fair potential function and $\nabla x_{r}\in\mathbb{R}^{2}$
is the gradient at voxel $x_{r}$. We implemented the Momentum 2 method
(Table IV in \cite{kim2015combining}) within the IRT toolbox. We
use the settings from the paper for the regularizer i.e. the Fair
potential with $\delta=10$, $a=0.0558$, and $b=1.6395$, and bit
reversal for subset ordering. We tweaked $\beta$ and $M$ \cite{kim2015combining}

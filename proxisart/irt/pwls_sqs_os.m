...
% malaa
arg.mom = 0; % momentum 0 means no momentum
...
if arg.mom == 2
  ...
  for iter=1:arg.niter
    ...
    for iset=1:nblock  
      % x
      zup = subset_update(z, iset);
      x = z + relax * zup;
      x = min(pixmax, max(pixmin, x));
      
      % v
      vsum = vsum + relax * t * zup;
      v = min(pixmax, max(pixmin, vsum));      
...
%       x = x + relax * num ./ den;	% relaxed update
      x = x + relax * subset_update(x, iset);
      x = min(pixmax, max(x,pixmin));		% lower bound
    end
...
%     info(iter,:) = arg.userfun(x, arg.userarg{:});
  ...
  % computes the numerator / denominator for a specific subset
  function sup = subset_update(x, iset)
		iblock = starts(iset);
		ia = iblock:nblock:na;

		li = Ab{iblock} * x;
    % malaa
		li = reshape(li, length(ia), nb);
